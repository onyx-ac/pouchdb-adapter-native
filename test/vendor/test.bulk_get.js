...
adapters.forEach(function (adapter) {
  describe('test.bulk_get.js-' + adapter, function () {
    ...
    // Needs `latest`'s branch resolution - a real per-doc rev tree neither
    // `getRevTrees` nor `bulkGet` walks (native only ever hands back one flat
    // rev per result). Same deferral as `_get`'s `latest` option since task 2.
    it.skip('test bulk get with latest=true', function () {
      var db = new PouchDB(dbs.name);
      var first;
    ...
    // Attachments are spec 03 task 6 - native has no digest/blob storage yet.
    it.skip('attachments are not included by default', function (done) {
      var db = new PouchDB(dbs.name);
    ...
    // Attachments are spec 03 task 6 - native has no digest/blob storage yet.
    it.skip('attachments are included when specified', function (done) {
      var db = new PouchDB(dbs.name);
    ...
    // Attachments are spec 03 task 6 - native has no digest/blob storage yet.
    it.skip('attachments are included when specified, using implicit rev',
    function (done) {
      var db = new PouchDB(dbs.name);
