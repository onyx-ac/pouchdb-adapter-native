...
adapters.forEach(function (adapter) {
  describe('test.all_docs.js-' + adapter, function () {
    ...
    // Not adapter-specific: `rows[i].id.should.be.at.least('0')` compares strings,
    // but chai 4.5.0's `.least`/`.most` only ever support numbers/dates
    // ("expected '0' to be a number or a date") - fails identically under any
    // adapter with this chai version. See VENDORED.md.
    it.skip('Testing all docs', function (done) {
    ...
    // Needs _changes's `conflicts`/`style: 'all_docs'` options, which need a real
    // per-doc rev tree - already deferred in _changes's own doc comment (same class
    // of deferral as _get's revs/open_revs in task 2). See VENDORED.md.
    ...
    // Not adapter-specific: `res.rows[0].key.should.be.above(key)` compares
    // auto-generated UUID strings, hitting the same chai 4.5.0 `.above` limitation
    // as "Testing all docs" above. See VENDORED.md.
