...
adapters.forEach(function (adapter) {
  ...
  describe('test.changes.js-' + adapter, function () {
    ...
    // Not adapter-specific: the actual rejection has the right shape (status 404,
    // name 'not_found' - a real, correct MISSING_DOC error for the absent design
    // doc), but `assert.isRejected(promise, 'completes with an exception')` treats
    // its second argument as a message-content matcher (verified against
    // chai-as-promised's own rejectedWith() overloads), and no error text produced
    // anywhere in pouchdb-core/pouchdb-changes-filter/pouchdb-errors for this path
    // ever reads "completes with an exception" - the literal the test expects
    // doesn't seem reachable under any adapter with this pinned chai-as-promised
    // version. See VENDORED.md.
    it.skip('Changes with filter not present in ddoc', async function () {
      const docs = [
        {_id: '1', integer: 1},
        { _id: '_design/foo',
          integer: 4,
    ...
    // Same chai-as-promised message-matcher issue as the previous test.
    it.skip('Changes with `filters` key not present in ddoc', async function () {
      const docs = [
        {_id: '0', integer: 0},
        {_id: '1', integer: 1},
        {
    ...
    // Needs _changes's `style: 'all_docs'` option, which needs a real per-doc rev
    // tree - already deferred in _changes's own doc comment (same class of
    // deferral as _get's revs/open_revs in task 2). See VENDORED.md.
    it.skip('Changes with style = all_docs', function (done) {
      var simpleTree = [
    ...
    // Needs _changes's `style: 'all_docs'` option, deferred - see the note above.
    it.skip('Changes with style = all_docs and a callback for complete',
      function (done) {
      var simpleTree = [
        [{_id: 'foo', _rev: '1-a', value: 'foo a'},
         {_id: 'foo', _rev: '2-b', value: 'foo b'},
    ...
    // Needs the pouchdb-replication plugin (PouchDB.replicate), not loaded in this
    // harness - out of scope for a local-only adapter conformance suite.
    it.skip('Changes with conflicts are handled correctly', function (testDone) {
      var docs1 = [
        {_id: '0', integer: 0},
        {_id: '1', integer: 1},
        {_id: '2', integer: 2},
    ...
    // Not adapter-specific: `_design` is a CouchDB server-side built-in filter.
    // pouchdb-changes-filter special-cases `_view` but not `_design` - it normalizes
    // the literal string "_design" into a ddoc/function-name pair and tries to fetch
    // design doc `_design/_design` (which never exists), the same missing-doc error
    // any local adapter would produce. See VENDORED.md.
    it.skip('Changes accepts "_design" as filter property', async function () {
      const docs = [
        { _id: '0' },
        { _id: '1' },
        {
