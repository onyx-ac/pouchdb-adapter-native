...
adapters.forEach(function (adapter) {
  ...
  describe('test.basics.js-' + adapter, function () {
    ...
    // Needs the pouchdb-replication plugin (db.replicate), not loaded in this
    ...
    // Needs the pouchdb-replication plugin, not loaded in this harness. See above.
    ...
    // Structurally can't pass under any adapter: it calls `db.put(doc)` with no
    // trailing callback, then asserts the plugin's wrapper saw one
    // (`arguments[arguments.length - 1]` is the doc, never a function) - a legacy
    // `initPull`/`cleanupPut` replication-hook pattern pouchdb-core@9 doesn't wire
    // up this way anymore. Not adapter-specific.
