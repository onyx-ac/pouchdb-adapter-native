...
adapters.forEach(function (adapter) {
  describe('test.get.js-' + adapter, function () {
    ...
    // Needs _get's `latest` option (walk to a branch's current leaf instead of the
    // literal requested rev) - explicitly deferred in src/index.ts's _get doc
    // comment since spec 03 task 2 ("`revs`/`open_revs`/`conflicts` options need
    // real multi-revision trees to mean anything and are follow-up work"), not a
    // new gap this task found. See VENDORED.md.
    it.skip('5857 - open_revs with latest=true', function () {
      var db = new PouchDB(dbs.name);
      var first = null;
    ...
    // Needs _get's `latest` option. See the note above and VENDORED.md.
    it.skip('5857 - multiple open_revs for the same branch with latest=true returns one result', function () {
      var db = new PouchDB(dbs.name);
      var first = null;
      return db.post({ version: 'first' })
        .then(function (info) {
    ...
    // Needs _get's `latest` option. See the note above and VENDORED.md.
    it.skip('5857 - GET old revision with latest=true', function () {
      var db = new PouchDB(dbs.name);
      var first = null;
      return db.post({ version: 'first' })
        .then(function (info) {
    ...
    // Needs _get's `latest` option. See the note above and VENDORED.md.
    it.skip('5857 - GET open_revs losing, old revision with latest=true', function () {
      var db = new PouchDB(dbs.name);
      var doctree = [
        {
          _id: 'mydoc',
    ...
    // Needs _get's `latest` option. See the note above and VENDORED.md.
    it.skip('5857 - GET open_revs losing and winning branches with latest=true', function () {
      var db = new PouchDB(dbs.name);
      var doctree = [
        {
          _id: 'mydoc',
